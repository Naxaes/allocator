...
#define ALLOCATOR_PRE_REALLOCATE_HOOK(allocator, memory, new_size, alignment, file, function, line) size_t start_time = time_ns()
...
int allocation_hook_init(const char* path) {
    ...
    if (clock_getres(CLOCK_UPTIME_RAW, &resolution) != 0) {
        return 1;
    }

    size_t resolution_ns = resolution.tv_nsec;
